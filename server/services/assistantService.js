...
function hasOpenAiKey() {
  ...
  return apiKey.length > 0 && !apiKey.startsWith('your-') && !apiKey.startsWith('replace-with-')
}
...
function buildFallbackResponse(message, context) {
  ...
  if (lowerMessage.includes('price') || lowerMessage.includes('plan') || lowerMessage.includes('buy')) {
    return withSource(
      {
        reply: `${SITE_DISPLAY_NAME} has three buying lanes: Operator OS for direct software checkout, Launch Sprint for guided implementation, and Enterprise Deployment for scoped rollout. Active payment routing is ${providerList}.`,
        suggestions: [
  ...
  if (lowerMessage.includes('deploy') || lowerMessage.includes('launch') || lowerMessage.includes('publish')) {
    return withSource(
      {
  ...
  if (lowerMessage.includes('paypal') || lowerMessage.includes('payment')) {
    return withSource(
      {
        reply: `PayPal checkout is available and can route through mr.jwswain@gmail.com when API credentials are not present. Stripe and PayPal readiness are surfaced directly on the pricing and offer cards.`,
        suggestions: [
  ...
  return withSource(
    {
      reply: `${SITE_DISPLAY_NAME} is built to turn natural-language requests into execution across content, offers, and deployment operations. Right now the platform is tracking ${context.analytics.visitors ?? 0} visitors, ${context.analytics.leads ?? 0} leads, and ${context.analytics.purchases ?? 0} purchases.`,
      suggestions: [
